...
do
		...
		sumtotal_time_j=$(grep " $j "  energy-100-500.dat | awk -v column=${column} '{print $column}' | awk '{ sum += $1; } END { print sum; }')

#                echo $sumtotal_time_j  

		no_traj_available_j=$(grep " $j "  energy-100-500.dat | awk -v column=${column} '{print $column}' | wc -l)
...
paste time.dat av$state.dat | column -s $'\t' -t >> av-au-$state-energy.dat
